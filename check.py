...
def e_number (db_address,table_name2):
    # 讀sql
    ...
    # 添加一個新欄位來檢查是否符合格式
    df_sql['e_number_check'] = df_sql['Customer Id'].str.match(pattern)
...
def FAN_number (db_address,table_name2):
    # 讀sql
    with sqlite3.connect(db_address) as conn:
        df_sql = pd.read_sql(f"SELECT * FROM {table_name2}", conn)
        conn.commit()
    ...
    # 添加一個新欄位來檢查是否符合格式
    df_sql['FAN_number_check'] = df_sql['Customer Id'].str.match(pattern)
